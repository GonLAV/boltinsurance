...
async function createTestCase(orgUrl, project, title, description, steps) {
  try {
    ...
    Logger.success('Created Test Case via isolated integration', { id: resp.data.id, title });
 ...
 */
async function findOrCreateTestCase({ orgUrl, project, title, description, steps = [], planId, suiteId }) {
  if (!PAT) {
    Logger.error('AZDO_PAT missing - cannot perform Azure DevOps operations');
    return { success: false, error: 'NO_PAT', message: 'AZDO_PAT not configured in environment' };
  }
  ...
  // Not found -> create
  const created = await createTestCase(orgUrl, project, title, description, steps);
  if (!created.success) {
