...
exports.getUserStories = async (req, res) => {
  ...
  try {
    ...
    const cacheKey = `userStories:${orgUrl}:${effectiveProject}:${areaPath || ''}:${iterationPath || ''}:${String(top || '')}:${patHash}`;
    ...
    const result = await svc.getAllUserStories(top, areaPath, iterationPath);
    ...
    const cacheKey = `userStories:${orgUrl}:${project || ''}:${areaPath || ''}:${iterationPath || ''}:${String(top || '')}:${patHash}`;
...
exports.getStoriesWithoutTests = async (req, res) => {
  ...
  try {
    const crypto = require('crypto');
    const patHash = pat ? crypto.createHash('sha256').update(pat).digest('hex').slice(0, 16) : 'ntlm';
    const cacheKey = `userStories:notests:${orgUrl}:${effectiveProject}:${areaPath || ''}:${String(top || '')}:${patHash}`;
    const cached = cache.get(cacheKey);
    ...
    const svc = new AzureDevOpsService(orgUrl, effectiveProject, pat);
    const result = await svc.getStoriesWithoutTests(top, areaPath);
    if (result && result.success) {
...
exports.getStoriesWithTests = async (req, res) => {
  ...
  try {
    const crypto = require('crypto');
    const patHash = pat ? crypto.createHash('sha256').update(pat).digest('hex').slice(0, 16) : 'ntlm';
    const cacheKey = `userStories:hastests:${orgUrl}:${effectiveProject}:${areaPath || ''}:${String(top || '')}:${patHash}`;
    const cached = cache.get(cacheKey);
    if (cached) return res.status(200).json(cached);
    const disk = diskCache.get(cacheKey, config.cache.ttlSeconds);
    if (disk.exists && disk.valid) return res.status(200).json(disk.value);
    ...
    const svc = new AzureDevOpsService(orgUrl, effectiveProject, pat);
    const result = await svc.getStoriesWithTests(top, areaPath);
    if (result && result.success) {
      cache.set(cacheKey, result, 60);
      diskCache.set(cacheKey, result);
    }
...
exports.getTestCasesBasic = async (req, res) => {
  ...
  try {
    ...
    const limit = Number.isFinite(parsedTop) && parsedTop > 0 ? parsedTop : 200;

    const areaClause = areaPath
      ? `AND [System.AreaPath] UNDER '${String(areaPath).replace(/'/g, "''")}'`
      : '';
    const wiql = `SELECT [System.Id], [System.ChangedDate]
                  FROM WorkItems
                  WHERE [System.TeamProject] = '${effectiveProject}'
                    ${areaClause}
    ...
    const batch = await svc.getWorkItemsBatch(ids, [
      'System.Id',
      'System.Title',
      'System.State',
      'System.CreatedDate',
    ...
    const testCases = (batch.data?.value || [])
